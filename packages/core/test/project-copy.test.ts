...
describe("ProjectCopy", () => {
  ...
  it.effect("detects registered strategies in insertion order", () =>
    Effect.gen(function* () {
      const copy = yield* ProjectCopy.Service
      const directory = abs("/tmp/project-copy-registry-order")
      const first = ProjectCopy.StrategyID.make("acme/first")
      const second = ProjectCopy.StrategyID.make("acme/second")
      const strategy = (id: ProjectCopy.StrategyID): ProjectCopy.Strategy => ({
        id,
      ...
      yield* copy.register(strategy(first))
      yield* copy.register(strategy(second))

      expect(yield* copy.detect({ directory })).toBe(first)
    }),
  )
  ...
  it.live("detects linked git worktrees but not root checkouts", () =>
    Effect.gen(function* () {
      ...
      expect(yield* copy.detect({ directory: target })).toBe(gitWorktree)
    }),
  )
  ...
  it.live("refresh keeps missing ordinary checkouts without failing", () =>
    Effect.gen(function* () {
      const input = yield* setup()
      ...
      expect(yield* stored(input.projectID)).toContainEqual({ directory: missing, strategy: null })
    }),
  )
})
